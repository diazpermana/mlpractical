...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        encoded = np.zeros((target.size, 10), dtype=int)
        encoded[np.arange(target.size), target] = 1
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        for i in range(num_windows * window_size):
            inputs.append(z[i:i+window_size-1])
            targets.append(z[i+window_size-1])
        ...
        # targets = ...
        input_batches = np.array([inputs[i:i+batch_size] for i in range(0, len(inputs), batch_size)])
        target_batches = np.array([targets[i:i+batch_size] for i in range(0, len(targets), batch_size)])
