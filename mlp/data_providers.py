...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        """
        target = np.array(int_targets)
        encoded = np.zeros((target.size, 10), dtype=np.float32)
        encoded[np.arange(target.size), target] = 1.0
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        usecols = tuple(range(2,33,1))
        data_path = 'D:\Courses\MLP\Labs\~\mlpractical\data\HadSSP_daily_qc.txt'
        data = np.loadtxt(data_path, skiprows=3, usecols=usecols)
        # print(data)
        #TODO: filter out all missing datapoints and flatten to a vector
        filter_data = data > -99
        filtered_data = data[filter_data]
        
        #TODO: normalise data to zero mean, unit standard deviation
        x_mean = np.mean(filtered_data)
        x_std = np.std(filtered_data)
        z = (filtered_data - x_mean)/x_std

        # #TODO: convert from flat sequence to windowed data

         # Generate overlapping windows
        num_windows = len(z) - window_size + 1
        windows = np.array([z[i:i + window_size] for i in range(num_windows)])
        # print(windows.shape)
        # Separate into inputs and targets
        inputs = windows[:, :-1]  # First `window_size - 1` elements
        targets = windows[:, -1]  # Last element
        ...
        input_batches = np.array_split(inputs, len(inputs) // batch_size)
        target_batches = np.array_split(targets, len(targets) // batch_size)


        for batch in input_batches:
            input_batches = np.vstack(batch)
        for batch in target_batches:
            target_batches = np.vstack(batch)

        target_batches = np.reshape(target_batches, (3,))

        # initialise base class with inputs and targets arrays (uncomment below)
        super(MetOfficeDataProvider, self).__init__(
            input_batches, target_batches, batch_size, max_num_batches, shuffle_order, rng)
    def __next__(self):
            return self.next()
